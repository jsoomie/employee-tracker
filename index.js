...
// Breaks line
const linebreak = (symbol = "-", repeatTime = 35) =>
...
const start = () => {
    ...
    inquirer
        .prompt({
            ...
            choices: ["Employee Management", "EXIT"],
        })
        .then((answers) => {
            ...
            .then((answers) => {
                ...
                db.query(
                    ...
                    (err, res) => {
                        if (err) throw err;
                        // Maps query
                        const itemsManager = res.map(
                            (item) => `${item.id} ${item.employee}`
                        );
                        inquirer
                            .prompt([
                                {
                            ...
                            .then((answersManager) => {
                                ...
                                const managerID = `${answersManager.addManager}`.split(
                                    " "
                                )[0];
                                // UPdates the employee with the manager
                                db.query(
                                    `UPDATE employee SET manager_id = ${managerID} WHERE CONCAT(employee.first_name, ' ', employee.last_name) = "${firstName} ${lastName}"`
                                );
                                console.log(
...
const updateEmployeeRole = () => {
    ...
    db.query(`SELECT * FROM employee;`, (err, res) => {
        ...
        const employeeFullName = res.map(
            (fullName) =>
                `${fullName.id} ${fullName.first_name} ${fullName.last_name}`
        );
        inquirer
            .prompt([
                {
                    ...
                    name: "employee",
                    message:
                        "Please first choose an employee to adjust their role",
                        ...
                        .prompt([
                            {
                                ...
                                name: "role",
                                message: (answer) =>
                                    `Please choose the role for employee ${employeeChoice}`,
                                choices: roleQuery,
                            },
                        ])
                        .then((answerR) => {
                            ...
                            db.query(
                                `UPDATE employee SET employee.role_id = ${roleChoice} WHERE employee.id = ${employeeChoiceID}`
                            );
                            console.log(
                                `Updating ${employeeChoice} with the role of ${roleName}`
                            );
                            linebreak();
                            manage();
                        });
                    ...
                    .prompt([
                        {
                            ...
                            name: "managerChoice",
                            message: (answer) =>
                                `Please choose the new manager for ${employeeName}`,
                            choices: query,
                        },
                    ])
                    .then((answers) => {
            ...
            .prompt(
                {
                    ...
                    name: "deptname",
                    message:
                        "Please Choose Which Department You Wish To Remove: ",
                    choices: choices,
                },
                {
                    type: "confirm",
                    name: "confirm",
                    message: (answer) =>
                        `Do You Wish To Delete ${answer.deptname}?`,
                }
            )
            .then((answer) => {
                // switch to confirm
                switch (answer.confirm) {
                    ...
                    default:
                        ...
                        db.query(
                            `DELETE FROM department WHERE department.name = "${answer.deptname}";`
                        );
                        linebreak();
                        manage();
                        break;
        ...
        inquirer
            .prompt([
                ...
                },
                {
                    ...
                    name: "department",
                    message:
                        "Please Enter Which Department this role is associated with",
                    choices: viewDepts,
                },
                {
                    type: "confirm",
                    name: "confirmation",
                    message: (answer) =>
                        `CONFIRM: Create a new role called ${answer.newRole}?`,
                },
            ])
            .then((answers) => {
                //switch confirm
                switch (answers.confirmation) {
                    case true:
                        ...
                        db.query(
                            `INSERT INTO role(title, salary, department_id) VALUES ("${role}", ${salary}, ${idDept})`
                        );
                        console.log(
        ...
        inquirer
            .prompt([
                ...
                },
                {
                    ...
                    name: "confirmation",
                    message: (answer) =>
                        `CONFIRM: Remove ${answer.rolesChoice} from the roster?`,
                },
            ])
            .then((answers) => {
